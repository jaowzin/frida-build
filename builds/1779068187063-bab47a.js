...
(function(Il2Cpp3) {
  function dump(fileName, path) {
    ...
    for (const assembly of Il2Cpp3.domain.assemblies) {
      ...
      for (const klass2 of assembly.image.classes) {
        file.write(`${klass2}
...
function addFlippedEntries(obj2) {
  return Object.keys(obj2).reduce((obj3, key) => (obj3[obj3[key]] = key, obj3), obj2);
}
...
function klass(img, ...names) {
  ...
  throw new Error(`Classe n\xE3o encontrada: ${names.join(" | ")}`);
}
function method(c, name, argc) {
  ...
  if (!found) {
    throw new Error(`M\xE9todo n\xE3o encontrado: ${c.name}.${name}/${argc ?? "*"}`);
  }
  return found;
}
function obj(self) {
...
function setField(o, name, value) {
  try {
    o.field(name).value = value;
    return true;
  } catch (_) {
    return false;
  }
...
function getField(o, name) {
  try {
    return o.field(name).value;
  } catch (_) {
    return null;
  }
}
function invokeInstance(self, name, argc, ...args) {
  return obj(self).method(name, argc).invoke(...args);
}
function listCount(listObj) {
  try {
    return listObj.method("get_Count").invoke();
  } catch (_) {
    try {
      return listObj.field("_size").value;
    } catch (_2) {
      return 0;
    }
  }
}
function listGet(listObj, index) {
  try {
    return listObj.method("get_Item").invoke(index);
  } catch (_) {
    try {
      const items = listObj.field("_items").value;
...
function patchFPSPlayer(player) {
...
function patchHealthScriptIfPlayer(h) {
...
function patchLivePlayers(img) {
  try {
    const FPSPlayer = klass(img, "FPSPlayer");
    for (const p of Il2Cpp.gc.choose(FPSPlayer)) {
      patchFPSPlayer(p);
    }
  } catch (_) {
  }
  try {
    ...
    for (const h of Il2Cpp.gc.choose(HealthScript)) {
      patchHealthScriptIfPlayer(h);
    }
  } catch (_) {
  }
}
function unlockSkinItem(item) {
  if (!item || item.isNull()) {
    return;
  }
...
function unlockSkinsRuntimeOnly(img) {
  try {
    const WeaponSkinManager = klass(img, "WeaponSkinManager");
    const instance = WeaponSkinManager.field("Instance").value;
    if (!instance || instance.isNull()) {
      return;
    }
    const skins = instance.field("weaponSkins").value;
...
Il2Cpp.perform(() => {
  ...
  try {
    ...
    start.implementation = function(...args) {
  ...
  try {
    ...
    damage.implementation = function(...args) {
      const self = obj(this);
    ...
    reduce.implementation = function(...args) {
      const self = obj(this);
      const aiBase = getField(self, "myAIBaseScript");
      if (!aiBase || aiBase.isNull()) {
        patchHealthScriptIfPlayer(self);
    ...
    deathCheck.implementation = function(...args) {
      const self = obj(this);
      const aiBase = getField(self, "myAIBaseScript");
      if (!aiBase || aiBase.isNull()) {
        patchHealthScriptIfPlayer(self);
  ...
  try {
    ...
    getCurrency.implementation = function(...args) {
    ...
    setCurrency.implementation = function(...args) {
      const type = Number(args[0]);
  ...
  try {
    ...
    findSkins.implementation = function(...args) {
      const skin = invokeInstance(this, "FindSkins", 1, ...args);
      unlockSkinItem(skin);
      return skin;
    };
  ...
  try {
    ...
    init.implementation = function(...args) {
      const item = args[0];
      unlockSkinItem(item);
